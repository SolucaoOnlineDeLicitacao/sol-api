...
RSpec.describe ProvidersService::Unblock, type: :service do
  let(:user) { create(:admin) }
  let(:provider) { create(:provider, type: 'Provider', blocked: true) }
  let(:comment) { 'a comment' }
  let(:params) { { creator: user, provider: provider, comment: comment } }
  let(:event) do
    create(:event_provider_access, eventable: provider, creator: user, blocked: 0)
  end
  let(:event_response) { double('call!', call!: true, event: event) }

  before do
    allow(EventServices::Provider::Unblock).
  ...
  describe '.call' do
    ...
    context 'when success' do
      ...
      it do
        expect(EventServices::Provider::Unblock).
