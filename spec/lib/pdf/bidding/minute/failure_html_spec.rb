...
RSpec.describe Pdf::Bidding::Minute::FailureHtml do
  ...
  describe '.call' do
    ...
    context 'when able to generate' do
      ...
      let!(:bidding) { create(:bidding, build_lot: false, lots: [lot_1, lot_2], kind: kind, status: :failure) }
      let!(:event) { create(:event_bidding_failure, eventable: bidding) }
      let(:kind) { :global }
